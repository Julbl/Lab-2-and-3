        ...
        return(
            <>
                <Col span={8} style={{margin: '0 auto'}}>
                    <h3>My todo list{props.uid ? '#' + props.uid : ''}</h3>
                    <List>
                        <Input placeholder='Add to todolist' onPressEnter={(e) => HandlerAddToTodo(e)}/>
                        {pageList.map((item,index)=>{
                            return(
                                    ...
                                    <List.Item key={index} style={{listStyle: 'decimal', textDecoration : item.completed ? 'line-through' : 'none', display: 'flex'}} >
                                        ...
                                        <Button onClick={()=> HandlerDeleteTodo(item.id) }>Delete</Button>
                                    </List.Item>
                            )
                        })}
                    </List>
